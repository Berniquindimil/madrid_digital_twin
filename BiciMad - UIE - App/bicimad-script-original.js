...
function getOccupancyClass(station) {
    if (!station.activate || station.activate === 0) {
...
function updateStats(stations) {
    const activeStations = stations.filter(s => s.activate === 1);
    ...
    activeStations.forEach(s => {
        if (s.total_bases > 0) {
            totalOccupancy += (s.dock_bikes / s.total_bases);
        }
    });
...
// Filtros
document.getElementById('filter-bikes').addEventListener('change', () => {
    updateMap(allStations);
});

document.getElementById('filter-bases').addEventListener('change', () => {
    updateMap(allStations);
});
...
document.getElementById('export-csv').addEventListener('click', () => {
    ...
    allStations.forEach(station => {
        ...
        csv += `${station.activate === 1 ? 'Sí' : 'No'},`;
        csv += `${station.geometry.coordinates[1]},`;
        csv += `${station.geometry.coordinates[0]},`;
        csv += `"${station.address || ''}"\n`;
    });
